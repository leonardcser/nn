...
// Forward declare linalg types if they were to be used in these headers
// directly For now, we assume linalg.h is included by .cpp files for math, and
// our ABI uses float* for dynamic data. #include "linalg.h"
...
// Input: z_values (vector before activation), Output: activations (vector after
// activation) Both vectors are of 'size'.
typedef void (*ActivationFunctionPtr)(const float *z_values, float *activations,
                                      int size);

// Activation Function Derivatives:
// Input: z_values (vector before activation), activations (vector after
// activation) Output: derivatives (d(activation)/dz) All vectors are of 'size'.
typedef void (*ActivationDerivativePtr)(const float *z_values,
                                        const float *activations,
                                        float *derivatives, int size);
...
typedef float (*ComputeLossPtr)(const float *predictions, const float *targets,
                                int size);
...
// For CrossEntropyWithSoftmax, this will be dL/dz_last_layer (predictions -
// targets).
typedef void (*LossDerivativePtr)(const float *predictions,
...
// - validation_loss: Average validation loss for the completed epoch (-1.0f if
// not applicable).
// - user_data: Arbitrary user data pointer passed from TrainConfig.
typedef void (*EpochCompletedCallback)(int epoch, float training_loss,
                                       float validation_loss, void *user_data);
...
typedef void (*LayerForwardPassFunc)(struct Layer *self,
                                     const float *input_data,
                                     float *output_data);

// `upstream_delta` is the error gradient from the next layer
// (dL/da_current_layer). `computed_delta_for_prev_layer` is the error gradient
// this layer computes for the previous layer (dL/da_prev_layer).
// `last_input_data_for_this_layer` is the `input_data` that was fed to this
// layer during the corresponding forward pass.
typedef void (*LayerBackwardPassFunc)(
    struct Layer *self, const float *upstream_delta,
...
typedef void (*LayerUpdateWeightsFunc)(struct Layer *self, float learning_rate,
                                       int batch_size);
...
struct __attribute__((packed)) Layer {
    LayerType type;
    int input_size;
    int output_size; // For Dense: output features. For Activation: input_size
                     // == output_size.
    ...
    ActivationDerivativePtr
        _activate_derivative_func; // e.g., derivative_sigmoid
    ...
    const float
        *_last_input_data; // Input this layer received (size: input_size)
    const float *_last_output_activations; // Output this layer produced (size:
                                           // output_size)

    // --- Layer-specific function pointers ---
    LayerForwardPassFunc _forward_pass;
    LayerBackwardPassFunc _backward_pass;
    LayerUpdateWeightsFunc _update_weights; // NULL if layer has no weights
                                            // (e.g., activation layers)
    LayerZeroGradientsFunc
        _zero_gradients; // NULL if layer has no gradients to zero
...
struct __attribute__((packed)) Model {
    ...
    // These buffers are used to pass data between layers during forward and
    // backward passes to avoid repeated allocations/deallocations. Their size
    // is determined by the maximum I/O size required by any layer.
    int _max_intermediate_io_size; // Max of all layer input/output sizes
                                   // (excluding model input/output buffers if
                                   // separate)

    float *
        _forward_buffer1; // Ping-pong buffers for activations/outputs of layers
    float *_forward_buffer2; // One buffer acts as input, the other as output,
                             // then they swap roles.

    float *_backward_buffer1; // Ping-pong buffers for delta values flowing
                              // backward
    float *_backward_buffer2;

    // Pointers to track which buffer is current input/output for
    // forward/backward pass These will point to forward_buffer1/2 or
    // backward_buffer1/2.
...
struct __attribute__((packed)) TrainConfig {
    ...
    int epochs;
    int batch_size;
    unsigned int random_seed; // For reproducible weight initialization and data
                              // shuffling
    int shuffle_each_epoch;   // Changed from bool to int (0 for false, 1 for
                              // true)

    LossFunctionType loss_type;
    ComputeLossPtr _compute_loss_func; // e.g., loss_mean_squared_error
    LossDerivativePtr
        _loss_derivative_func; // e.g., derivative_mean_squared_error
...
struct __attribute__((packed)) Dataset {
    ...
    float *_inputs_flat;  // Changed to float* as create_dataset copies and
                          // Dataset owns this memory
    float *_targets_flat; // Changed to float* as create_dataset copies and
                          // Dataset owns this memory
};
...
// - layer_def_params: An array of parameters (e.g., num_output_neurons for
// Dense, ignored for Activation) The function will chain the input/output
// dimensions of the layers.
Model *create_model(
    int model_input_dim, int num_model_layers,
    const LayerType *layer_def_types, // Array of LayerType for each layer
    const int *layer_def_params // Array of parameters (e.g., num_output_neurons
                                // for Dense, ignored for Activation)
);

void free_model(Model *model);

// Weight Initialization Strategies (call after create_model)
void initialize_weights_xavier_uniform(
    Model *model); // Good general purpose for symmetric activations (sigmoid,
                   // tanh)
void initialize_weights_he_uniform(
    Model *model); // Good for ReLU family activations
...
void free_dataset(Dataset *dataset); // Frees the Dataset struct and its
                                     // internally copied data.
...
// Performs a full backward pass and accumulates gradients in each layer's dW
// and db.
// - input_sample_for_forward: The input sample that was used for the
...
// Updates weights for all layers that have them, using accumulated gradients
// and learning rate. Typically called after processing a batch.
...
// on the same model. The caller should NOT free this pointer.
